...
// Get recent resources from people you follow
export async function getFeedResources(username) {
  const following = await getFollowing(username)
  if (!following || following.length === 0) return []
  const names = following.map(f => f.following)
  const { data } = await supabase
    .from('resources')
    .select('*')
    .in('submitted_by', names)
    .order('created_at', { ascending: false })
    .limit(12)
  return data || []
}
...
export async function addReply(commentId, username, reply) {
  const { data } = await supabase
    .from('replies')
    .insert([{ comment_id: commentId, username, reply }])
  return data
}